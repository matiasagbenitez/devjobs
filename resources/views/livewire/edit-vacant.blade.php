<form class="md:w-1/2 space-y-3" wire:submit.prevent='editVacant'>
    ...
    <div>
        ...
        <div class="my-5 w-80">
            ...
            <img src="{{ asset('storage/vacants/'.$image) }}" class="rounded-lg" alt="Current image">
        </div>

        <x-label for="image" :value="__('New image?')" />
        <x-input id="image" class="block mt-1 w-full" type="file" wire:model="newImage" accept="image/*" />

        <div class="my-5 w-80">
            @if ($newImage)
                <x-label for="New image" :value="__('New image')" />
                <img src="{{ $newImage->temporaryUrl() }}" class="rounded-lg" alt="Current image">
            @endif
        </div>
